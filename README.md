...
<head>
  ...
  <style>
    ...
    *, *::before, *::after {
    ...
    .control-btn:active::before { width: 300px; height: 300px; }
    .btn-start {
      background: linear-gradient(135deg, #4ecdc4, #44a08d);
      color: white;
      grid-column: 1 / -1;
    }
    .btn-start:disabled { background: #1a1a1a; color: var(--text-secondary); }
    .btn-pause {
      background: linear-gradient(135deg, var(--accent-color), #1e90ff);
      color: white;
    }
...
<body>
  <header class="app-header">
    <div class="header-content">
      <div class="app-logo"><div class="logo-icon">⏱</div><h1 class="app-title">STOPWATCH!⏱️</h1></div>
    </div>
  </header>
  <main class="main-container">
    <div class="stopwatch-app">
      ...
      <div class="lap-section" id="lapSection">
        <div class="lap-header"><div class="lap-title">Lap Times</div><div class="lap-count" id="lapCount">0 laps</div></div>
  ...
  <script>
    class ProfessionalStopwatch {
      constructor(){
        this.startTime=0;this.elapsedTime=0;this.timerInterval=null;this.lapTimes=[];this.lapNumber=0;
        ...
        this.updateDisplay();this.updateStatus('stopped','Ready to Start');
      }
      start(){
      ...
      reset(){
        clearInterval(this.timerInterval);this.timerInterval=null;
        this.elapsedTime=0;this.lapTimes=[];this.lapNumber=0;
      ...
      recordLap(){
        ...
        const lastTotal=this.lapTimes.length?this.lapTimes[this.lapTimes.length-1].totalTime:0;
        this.lapTimes.push({number:++this.lapNumber,totalTime:now,lapTime:now-lastTotal});
        this.updateLapDisplay();
      }
      updateDisplay(){
        const time=this.timerInterval?Date.now()-this.startTime:this.elapsedTime;
      ...
      updateLapDisplay(){
        ...
        [...this.lapTimes].reverse().forEach(l=>{
          ...
          el.innerHTML=`<div class="lap-info"><div class="lap-number">Lap ${l.number}</div><div class="lap-split">Split: ${lm}:${ls}.${lms}</div></div><div class="lap-time">${m}:${s}.${ms}</div>`;
